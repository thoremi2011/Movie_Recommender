...
def mean_pooling(token_embeddings: np.ndarray, attention_mask: np.ndarray = None) -> np.ndarray:
    """
    Applies mean pooling on token embeddings.
    
    If an attention mask is provided, it averages only over the valid tokens.
    
    Args:
        token_embeddings: A numpy array of shape (batch_size, sequence_length, embedding_dim)
        attention_mask: An optional numpy array of the shape (batch_size, sequence_length)
                        indicating which tokens should be included in the pooling.
    
    Returns:
        A numpy array of shape (batch_size, embedding_dim) containing the pooled embeddings.
    """
    if attention_mask is not None:
        # Expand the mask to match the dimensions of the token embeddings.
